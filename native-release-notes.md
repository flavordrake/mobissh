...
- **Editing a profile's host, port or username no longer takes the saved password with it.** A banner warns before you save; the old credential is kept, just not sent to the new server. (#1138) **Device-test:** edit a profile's host and confirm the warning, then re-enter the password.
- **Connect through a bastion.** A profile can now name another saved profile as its **jump host** (profile editor → Jump host), and the app connects through it the way `ssh -J` does — the bastion is a real SSH connection with its own credentials and its own host-key check, and a jumped profile shows a badge in the list. (#1183) **Device-test:** point a profile at a bastion you use and connect; you should be asked to trust the bastion's key the first time, then land on the target.
...
- **Create folders in the file browser.** New-folder button in the toolbar, plus a "New folder" entry in the long-press menu — long-press a folder to create inside it, a file to create alongside it (the menu says which). Invalid names are rejected inline; the server's own error (permission denied, already exists) is shown as-is. (#1133) **Device-test:** create a folder from both the button and the long-press menu.
- **Keybar characters go into the IME preview.** With the compose bar open, tapping `/`, `-` or `|` now inserts into what you're composing instead of firing at the shell; arrows, Home/End, PgUp/PgDn, Esc and ^C still drive the terminal. Enter sends what you've composed (and is still a plain Enter when the buffer is empty). (#1131) **Device-test:** open the IME preview, tap `/` mid-word, then Enter.
- **Every keybar tap now gives a haptic click** — `-`, `/`, `|`, arrows, Ctrl, Reset, all of them, whether the key lands in the terminal or the IME preview. Previously only a HELD arrow key clicked. **Device-test:** tap `-` with the IME preview open and closed.
...
- **Session menu → Reconnect now actually reconnects a live session.** Previously, on a session that still looked connected (stale tmux, stuck mouse mode, frozen shell) Reconnect silently did nothing; it now tears the connection down and re-attaches, which also makes tmux re-send its mouse settings. (#1136) **Device-test:** on a healthy connected session, Session menu → Reconnect — the terminal should briefly show reconnecting, then the prompt/tmux comes back.
- **"Bad tmux mode" reports now carry the evidence.** Mouse-mode changes, the automatic mode reset after a reconnect, and taps that fall through to the terminal are all recorded in the bug-report bundle. (#881) **Device-test:** if scrolling/tapping goes wrong in tmux again, file a report from that state — it should now explain itself.
- **Port forwards now persist by default.** The add form has a **"Save to profile"** checkbox, ticked by default, so a forward you add is remembered and arms on every connect to that profile — untick it for a one-off. Ad-hoc sessions (no saved profile) have no checkbox. The per-row star still shows which forwards are saved and lets you un-save one. **Device-test:** add a forward, disconnect, reconnect — it should be listed and working without re-adding.
- **Editing a profile no longer wipes its saved forwards or per-profile font.** Any Save in the profile editor (rename, key change, the migration "Save & connect" step) silently dropped saved forwards and the profile's font size/family — that is the other reason forwards "didn't stick". **Device-test:** save a forward, then rename the profile — the forward is still there.
- **IME "Fix" no longer fuses separate commands.** Fix now uses the terminal width to tell a wrapped row from a real line break: a line wider than the terminal, or the short tail after full rows, keeps its newline, and full rows rejoin exactly (no more `19 65` splits or `.rulessudo` joins). Text that never touches the terminal edge — e.g. an indented URL pasted from a PDF — is cleaned up as before. **Device-test:** paste the two-line udev command into the IME preview, tap Fix — two lines, all quotes intact, Enter runs both.
...
- **Re-enter a key right from the profile editor.** The "Using a stored key" note now has a **Re-enter** button — paste the material for that key without leaving the editor the migration banner dropped you in. (#1121)
- **Keys now show WHO they are.** Importing or re-entering a key derives its type + `SHA256:` fingerprint (shown in Settings → SSH keys), and tapping a key row shows its full public line with Copy — match it against `authorized_keys` to identify which key is which. (#1122) **Device-test:** re-enter a key, check its fingerprint matches `ssh-keygen -lf` on the source machine.
