...
- **Export anyway, minus the broken entries.** When some credentials can't be read, tick "Export anyway without these" — the backup includes everything else and records what was skipped, and after import those profiles keep their key names for easy re-entry. **Device-test:** export with the two unreadable entries skipped, then re-enter them at leisure.
