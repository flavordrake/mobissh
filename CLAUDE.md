...
- **Before submitting a PR, run the full test suite:** `npx tsc --noEmit && npx eslint public/ server/ tests/ src/ && npx playwright test`. The `webServer` config in `playwright.config.js` auto-starts the server. Fix all failures before submitting.
