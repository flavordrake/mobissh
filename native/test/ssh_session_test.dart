...
void main() {
  ...
  group('SshSessionController #551 reconnect policy', () {
    ...
    test('exponential backoff progression for transient (non-unreachable)', () {
      final c = SshSessionController(reconnectDelay: const Duration(seconds: 2));
      // attempt is 0-based; base 2s, factor 1.5, cap 30s.
      expect(c.reconnectDelayFor(0, unreachable: false),
          const Duration(milliseconds: 2000));
      expect(c.reconnectDelayFor(1, unreachable: false),
          const Duration(milliseconds: 3000));
      expect(c.reconnectDelayFor(2, unreachable: false),
          const Duration(milliseconds: 4500));
      expect(c.reconnectDelayFor(3, unreachable: false),
          const Duration(milliseconds: 6750));
      // Far out — must be capped at 30s, never exceed.
      expect(c.reconnectDelayFor(20, unreachable: false),
    ...
    test('unreachable uses a fixed 1.5s interval regardless of attempt', () {
      ...
      expect(c.reconnectDelayFor(0, unreachable: true),
          const Duration(milliseconds: 1500));
      expect(c.reconnectDelayFor(5, unreachable: true),
          const Duration(milliseconds: 1500));
      expect(c.reconnectDelayFor(40, unreachable: true),
          const Duration(milliseconds: 1500));
      c.dispose();
    });
    ...
    test('isUnreachableError matches host-unreachable patterns', () {
      ...
      expect(
          SshSessionController.isUnreachableError(SSHSocketError(
      ...
      expect(
          SshSessionController.isUnreachableError(SSHSocketError(
            const SocketException('reset',
                osError: OSError('reset', 104)),
          )),
          isFalse);
    });

    test('unreachable close sets lastErrorUnreachable + retries at 1.5s ceiling 60',
        () async {
      var attempts = 0;
      final c = SshSessionController(
      ...
      c.handleTransportClosed(SSHSocketError(
        const SocketException('No route to host',
            osError: OSError('No route to host', 113)),
      ));
      ...
      expect(c.lastErrorUnreachable, isTrue,
    ...
    test('clean server-initiated close while connected emits soft_disconnected '
      ...
      expect(states, contains(SshSessionState.softDisconnected),
          reason: 'clean disconnect while connected must surface soft_disconnected');
    ...
    test('clean close while NOT connected stays disconnected (no reconnect)',
        () async {
