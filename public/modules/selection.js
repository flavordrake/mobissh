...
export function initSelection() {
    ...
    termEl.addEventListener('click', (e) => {
        if (!_selectionActive)
            return;
        if (e.target.closest('#selectionChip'))
            return;
    ...
    function _showPasteIfClipboard() {
    ...
    function _touchToBufferPos(clientX, clientY) {
     ...
     */
    function _getLogicalLine(bufferRow) {
        const term = appState.terminal;
        if (!term)
            return { text: '', firstRow: bufferRow, cols: 1 };
     ...
     */
    function _selectableUnitAt(bufferRow, col) {
        const term = appState.terminal;
        if (!term)
            return [col, bufferRow, 1];
    ...
    function _wordAt(bufferRow, col) {
        const term = appState.terminal;
        if (!term)
            return [col, bufferRow, 1];
        const { text, firstRow, cols } = _getLogicalLine(bufferRow);
    ...
    function _startDragSelect(clientX, clientY) {
    ...
    function _extendDragSelect(clientX, clientY) {
        if (!_dragActive || !appState.terminal)
            return;
        _hideChip();
        const pos = _touchToBufferPos(clientX, clientY);
        if (!pos)
