...
Create the PR:
```bash
gh pr create --base main --head bot/issue-{N} --title "<issue title>" --body "$(cat <<'EOF'
## Summary
...
🤖 Generated by local develop agent
EOF
)"
```
...
```bash
gh issue comment {N} --body "PR opened: <pr-url>. Cycles: {count}/3. All gates passing. Tests added/updated: <list>."
```
...
```bash
gh issue comment {N} --body "Development agent failed after {count} cycles. Failure: <type>. See bot-attempts.md for details."
```
